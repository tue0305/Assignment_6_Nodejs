...
export const loadUsers = () => {
    return function (dispatch) {
        axios
            .get("http://localhost:8001/api/user/get-all-users")
            .then((res) => {
...
export const deleteUsers = (userId) => {
    return function (dispatch) {
        axios
            .delete(`http://localhost:8001/api/user/delete-user/${userId}`)
            .then((res) => {
                console.log(res);
...
export const addUsers = (user) => {
    return function (dispatch) {
        axios
            .post(`http://localhost:8001/api/user/signup`, user)
            .then((res) => {
                dispatch(userAdd());
                dispatch(loadUsers());
            })
...
export const getDetailUsers = (userId) => {
    return function (dispatch) {
        axios
            .get(`http://localhost:8001/api/user/detail-user/${userId}`)
            .then((res) => {
                console.log(res);
